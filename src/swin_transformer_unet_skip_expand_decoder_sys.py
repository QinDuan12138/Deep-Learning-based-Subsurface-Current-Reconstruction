...
class FinalPatchExpandSmooth(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=4, out_channels=None, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        ...
        self.upsample = nn.Sequential(
            nn.Upsample(scale_factor=dim_scale, mode='bilinear', align_corners=True),
    ...
    def forward(self, x):
        ...
        x = self.upsample(x)                    # B, out_channels, H*scale, W*scale
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpandSmooth(
                    input_resolution=(img_size // patch_size, img_size // patch_size),
                    dim_scale=4, dim=embed_dim, out_channels=embed_dim, norm_layer=norm_layer
                )
